...
serve(async (req) => {
    ...
    try {
        ...
        // 3. Call OpenRouter
        // Используем оригинальные имена для промпта если переданы
        const namesForPrompt = originalFileNames || storagePaths;
        const prompt = getBatchPrompt(namesForPrompt) + "\n\nIMPORTANT: Return ONLY valid JSON object. No extra text.";
        ...
        if (!aiResponse.ok) {
            throw new Error(`OpenRouter Error: ${aiData.error?.message || aiResponse.statusText}`);
        }

        if (!aiData.choices?.[0]?.message?.content) {
            throw new Error(`AI returned invalid response format: ${JSON.stringify(aiData)}`);
        }
        ...
        await supabase.from("logs").insert({
            ...
            description: `Processed ${storagePaths.length} files via Edge Function`
        });
        ...
        return new Response(JSON.stringify({
            error: err.message,
            details: "Please check Edge Function logs for more info"
