...
78. - ⚠️ ПРАВИЛО 3000: Если ты видишь число больше 3000 в поле мест — скорее всего, ты взял "штуки". Перепроверь документ! Количество мест редко превышает 3000.
79. - Пакетная проверка: Сумма всех quantity в товарах ДОЛЖНА быть строго равна totalPackages документа. Если в списке 30 строк, а ты извлек 9 — это ошибка. Извлекай ВСЕ.
...
═══════════════════════════════════════════
КРОСС-ПРОВЕРКА И КОНСИСТЕНТНОСТЬ (ОЧЕНЬ ВАЖНО):
═══════════════════════════════════════════
- ТЫ ВИДИШЬ ВСЕ ФАЙЛЫ СРАЗУ. Делай MERGE данных в единую картину.
- ВЕС: CMR обычно содержит более точный GROSS WEIGHT, чем инвойс.
- ТРАНСПОРТ: Приоритет у CMR, если в техпаспорте данные другие.
- КОНТРАГЕНТЫ: Если в Инвойсе "Buyer", а в CMR графе 2 — другое имя, Получателем (Consignee) бери того, кто в CMR.
- ТОВАРЫ: Извлекай КАЖДУЮ физическую строку. Сумма quantity/grossWeight в products ДОЛЖНА строго совпадать с totalPackages/totalWeight документа.
- Если в одном файле есть БИН, а в другом только адрес — объедини их.
`;;
export function getBatchPrompt(fileNames) {
