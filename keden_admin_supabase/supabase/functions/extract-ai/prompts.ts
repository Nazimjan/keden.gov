...
⚠️ ВАЖНО: Основные данные водителя (Имя, Фамилия, Номер документа) бери из ЗАГРАНПАСПОРТА.
66. ИИН ВОДИТЕЛЯ (12 цифр): Если ИИН нет в паспорте, ты МОЖЕШЬ взять его из Удостоверения личности или любого другого документа водителя.
67. Ищи 12 цифр в полях "IIN", "ЖСН", "Personal Number" или просто отдельной строкой.
...
- driver.iin: СТРОГО 12 цифр. Для граждан РК ищи на обратной стороне ID-карты или в паспорте (поля ИИН/ЖСН).
- firstName, lastName: Из загранпаспорта (ЛАТИНИЦЕЙ ЗАГЛАВНЫМИ).
- document.number: Номер ЗАГРАНПАСПОРТА.
...
- Кросс-проверка: Средняя часть номера (напр. 231025) — это СТРОГО дата в формате ДДММГГ (DDMMYY).
  * ПРИМЕР: В номере "398 55400-231025-..." блок "231025" означает: 23 - день, 10 - месяц, 25 - 2025 год. 
  * КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО интерпретировать первые две цифры как год (YYMMDD), если это свидетельство РК (начинается на 398).
...
📜 СВИДЕТЕЛЬСТВО О ДОПУЩЕНИИ (VEHICLE_PERMIT):
- document.number: полный номер (напр. 398 55400-231025-00049).
- Кросс-проверка: Средняя часть номера (напр. 231025) — это СТРОГО дата выдачи в формате ДДММГГ (DDMMYY).
  * ВАЖНО: 231025 означает 23.10.2025 (23 день, 10 месяц, 2025 год). НИКОГДА не интерпретируй это как 2023 год.
